...
- rollout for the wallet progress sync still needs Supabase deploy access in the active environment: the migration and edge-function files are prepared in repo, but `db push` / function deploy were blocked in this session because the local Supabase CLI has no `SUPABASE_ACCESS_TOKEN`

...
## 2026-05-29 admin settings button allowlist
- Added a frontend seeded-admin wallet allowlist for the Settings gear, so the red Admin Panel entry appears for the requested test wallets after wallet verification even if the pending-withdraw admin summary has not loaded.
- The `/admin` page and admin actions still require the server admin session check.
...
- Production builds no longer compute compressed-size reports, which keeps the VPS deploy from getting killed during Vite's gzip-size analysis step.
...
- Added server-side canonical package tables for bait, MON fishing nets, MON cube rolls, wallet check-in price, and premium-session price/cast count. The frontend now sends intent and tx hashes only for those flows.
- Wallet check-in now verifies the payment tx and is idempotent once the player has checked in for the current UTC day.
- Added a local `payment_transactions` ledger so payment tx hashes are reserved before verification, applied through a `verified` -> `applied` phase with the economy write, retried idempotently after `applied`, and cannot be reused across purchase endpoints or concurrent requests.
- `/verify-purchase` now requires the active wallet session token, and the frontend API helper attaches the stored wallet session to direct purchase verification calls.
- Failed payment reservations can be retried by the real tx owner instead of letting another wallet permanently strand a public tx hash.
- Deterministic MON reward sources are de-duped with a unique reward-source index; cube rewards, premium casts, fishing-net claims, fish sales, task claims, and grill cook/sell actions now run in server transactions with current player state re-read before mutation.
- Premium session cast rewards no longer trust client-authored reaction quality; the current server fallback resolves premium casts with server-owned `good` quality until a tokenized premium reaction challenge is added.
- Client-authored audit logs are marked as `client` source and are excluded from legacy payment tx reuse detection.
- Local client economy mutators in `useGameState` now no-op while the server economy mode is active, so current UI cannot fall back to local coins/bait/fish/rod/NFT mutation helpers.
- Legacy direct `grant_fishing_mon_reward`, `grant_leviathan_common_rod_bonus`, and manual `complete_premium_session` actions now return `410`; fishing MON rewards and premium completion are produced during server cast resolution.
- Public grill leaderboard write/delete routes are disabled; authenticated name-sync writes preserve existing server score/dish values, and cooking writes the cumulative server-owned grill score.
- Added `docs/server-authoritative-economy-2026-05-22.md` and expanded `scripts/ops/smoke-api.mjs` with direct tamper probes for underpriced packages, invalid tiers, missing check-in tx, direct reward grants, manual premium completion, public leaderboard writes/deletes, unauthenticated purchase verification, audit-log tx poisoning, duplicate/cross-endpoint/concurrent/already-applied payment tx reuse, duplicate/concurrent fishing-net claims, premium reaction-quality tampering, and duplicate cube reward application in local fake-payment mode.
- Validation: `node --check server/index.mjs`, `node --check scripts/ops/smoke-api.mjs`, `npm run verify`, `npm run ops:smoke` with local fake payments, and an in-app browser reload/cast check passed. `npm run verify` still reports the existing 11 lint warnings and build warnings about wallet/provider chunks/third-party pure annotations.
...
- Wallet-only economy paths remain wallet-only: MON rewards, purchases, premium sessions, check-ins, and withdrawals require a verified wallet, while guest fishing/cube/server player state works without wallet connection.
- VPS deploy now creates a timestamped `/opt/hookloot/data` archive under `/opt/hookloot/backups` before switching releases; the server keeps the latest 20 backups by default.
- Validation: `npm run typecheck`, `npm run build`, direct API guest persistence checks, in-app browser guest cast check, and `npm run ops:smoke` with guest restore assertions.

## 2026-05-22 rod rarity, MON unlock, cube rewards, and fishing MON pulls
- Added a shared base-rod config with stable `common_rod`, `rare_rod`, `epic_rod`, and `legendary_rod` ids, rarity tiers, rare-fish bonuses, MON unlock prices, fishing MON pull parameters, and cube drop weights. A hidden `legacy_gold_rod` keeps old level-4 saves readable.
- Shop, MON market, inventory rod cards, rod preview assets, and the guide now read rod names/rarities/balance from that config instead of duplicating core values in UI components.
- Common Rod is the free default rod. Rare, Epic, and Legendary rods are sold in Monad Shop, show owned/not-owned/not-enough-MON states, and cannot be bought again once the player already owns that tier or a higher one.
- Fishing now uses the selected unlocked rod instead of always implying the highest owned rod. The fish HUD rod badge opens an equip popover, invalid equipped rods fall back to `common_rod`, and locked rod selections are ignored.
- Successful wallet-linked catches can roll an extra MON pull after the fish result. Common Rod has no MON pull, Rare/Epic/Legendary rods use modest increasing chances and reward ranges, and the catch result panel shows the pulled MON amount when one lands.
- Catching `leviathan` with the active `common_rod` now grants the configured achievement bonus `epic_rod`; if that tier is already owned, verified wallets receive a configured 2 MON duplicate compensation instead. The result panel shows this as a separate Leviathan feat reward.
- `player-actions` now accepts `grant_fishing_mon_reward`, validates the equipped rod id/level and configured reward range, de-dupes reward source refs, credits `player_mon_rewards`, and refreshes the client MON summary after a successful credit.
- `player-actions` also accepts `grant_leviathan_common_rod_bonus`, validates the Leviathan/common-rod condition server-side, updates `rod_level`/`equipped_rod` for the Epic Rod reward, and de-dupes duplicate MON compensation source refs.
- The cube reward contract now supports `rod` prizes on the client and in the Supabase `player-actions` flow. Cube rolls can inject one rod tile only for rod tiers above the player's current unlocked rod, and the current config makes the actual rod hit chance about 0.2% before Rare/Epic/Legendary weighting.
- Cube rod duplicate handling now grants configured MON compensation for verified wallets instead of re-awarding the same non-stackable rod, and the cube result toast names the duplicate bonus.
- The player guide now explains Monad Shop rod purchases, active rod selection, paid-rod MON pulls, the Common Rod Leviathan bonus, and rare cube rod drops in the existing `/guide` help flow. Shared content pages now hide the boot loader immediately so the guide is visible outside the main game screen.
- MON rod purchase verification now matches the current client prices for base rod unlocks and NFT rod mints, and audit metadata includes the stable rod id.
- Validation: `npm run typecheck`, `npm run build`, `npm run lint`, and `npm run verify` passed locally. `npm run lint` still reports 11 existing warnings in older UI/canvas files.
...
- 2026-05-22: Supabase runtime dependency is removed from the VPS path. The frontend now invokes same-origin Hook & Loot API routes, nginx proxies `/api/*` to the new `hookloot-api` container, local data lives in SQLite under `/data`, and ops smoke scripts target the owned API instead of Supabase Edge Functions.
- 2026-05-22: Verified-wallet core fishing economy is now server-authoritative on the owned API: normal cast start consumes bait server-side, reel/timeout resolves catch/miss server-side, XP/level/inventory/collection first-catch rewards/MON rod drops are stored on the server, verified fish sell / bait buy / coin rod buy / rod equip actions now call `player-actions`, and `save-player-progress` no longer accepts client-authored economy fields that could overwrite server results.
- 2026-05-22: Verified-wallet task and weekly progress moved into the owned API as well. Server fishing catch resolution now advances `catch_10`, `rare_1`, `catch_60_fish`, and `catch_6_rare`; server shop/grill/premium/referral/check-in actions advance their matching progress; `claim_task_reward` now validates server progress instead of trusting `task_progress_sync`; and cube rolls are consumed from server `dailyWheelRolls` / `paidWheelRolls`.
- 2026-05-22: The remaining verified MON shop fallbacks now route through the owned API: fishing net deployment/collection, extra cube-roll purchases, coin buys, NFT rod mints, MON rod unlocks, and wallet check-in all apply returned server player snapshots instead of mutating verified economy locally. The old client-only wallet check-in localStorage fallback and `task_progress_sync` claim payload are removed from the verified runtime path.
- 2026-05-22: Guest/local economy play is now disabled in the main game surface. Fishing, shop, tasks, grill, cube, inventory sell/equip, premium sessions, and economy tabs require a verified wallet session, and `useGameState` / `useGameProgress` can now run with client local player/progress storage disabled so the Hook & Loot screen no longer loads or writes local economy saves while using the owned API path.
- 2026-05-22: Frontend runtime calls no longer use the old third-party namespace. Purchases, wallet saves, player actions, admin, inbox, MON, and audit calls now import the owned `src/lib/serverApi.ts` helper directly.
- 2026-05-22: The frontend `src/` tree no longer carries Supabase-named runtime or type modules. The old compatibility client was removed, generated table shapes moved to `src/types/serverDatabase.ts`, and remaining app imports now point at neutral owned-server modules.
- 2026-05-22: Repo workflow/docs were moved onto the owned API path. GitHub Pages build no longer injects old backend env keys, VPS env sync only copies explicit public game flags plus Hook & Loot server vars, ops smoke commands point at `scripts/ops/smoke-api.mjs` / `smoke-readonly.ps1`, and README/AGENTS/VPS/ops handoff docs now describe `hookloot-api` + SQLite as the runtime source of truth.
- 2026-05-22: The tracked legacy Supabase source tree and stale Bun lockfile were removed. The repo now keeps the owned Node API/SQLite backend as runnable server code and uses the npm/package-lock workflow for dependency resolution.
- 2026-05-22: Local Supabase env residue was cleared as well. `.env` / `.env.local` no longer contain `VITE_SUPABASE_*` or `SUPABASE_*` keys, and the old local Supabase setup note was removed.
- 2026-05-22: Temporary test activity logging is now enabled by default for the owned API rollout. The client records session starts, screen opens, API successes/failures, blocked actions, and client/screen errors into the existing `player_audit_logs` path when a wallet or guest session is available, while also keeping a browser-local trace at `window.__hookLootTestActivityEvents`; the Node API now prints lightweight request and client-activity lines to stdout. Disable later with `VITE_TEST_ACTIVITY_LOGS_ENABLED=0` and `HOOKLOOT_TEST_ACTIVITY_LOGS_ENABLED=0`.
- 2026-05-22: Validation for the temporary logging/save check passed locally: `npm run typecheck`, `node --check server/index.mjs`, `npm run build`, `npm run lint`, `npm run ops:smoke`, and a focused `log-player-event` smoke all completed successfully. `npm run lint` still reports the existing 11 warnings only, and the Vite build still reports the known chunk / third-party annotation warnings.
- 2026-05-22: Repeated persistence validation passed on a fresh temporary SQLite database. A 3-round wallet/guest matrix confirmed nickname saves, wallet/guest restore after `save-player-progress`, server-authoritative bait/XP/catch/inventory persistence after cast start/resolve, guest-to-wallet carryover, linked-guest rotation, and rejection of client-authored economy/task tampering; `npm run ops:smoke` then passed 3 additional times against the same temp API. A browser pass on local Vite confirmed the guest client starts, `Cast line` calls the API, reload restores the same guest session, activity logs write through `log-player-event`, and no client console errors or screen-error UI appeared.
- 2026-05-23: Grill recipe cards now show themed ready-grill dish icons built from the existing fish artwork, and the cooking/result modal uses the same dish icon treatment instead of generic-only flame/trophy art.
- 2026-05-23: Grill recipe icons were redrawn as actual cooked-dish SVG miniatures: grill grate, flames, skewered fish, grilled fillets/steaks, platter variations, char marks, and a distinct cosmic grilled fish treatment replace the placeholder ingredient-style fish icons.
- 2026-05-23: Temporary Monad shop test mode is wired for local QA. The client can fake MON payment hashes in dev/test (`VITE_MONAD_SHOP_TEST_MODE_ENABLED`, default dev-only), the owned API can accept those payments and guest sessions only when `HOOKLOOT_MONAD_SHOP_TEST_MODE_ENABLED=1`, and `HOOKLOOT_MONAD_TEST_DROPS_ALWAYS=1` makes MON cube/premium/rod drops deterministic while testing paid rods and MON rewards.
- 2026-05-23: Inventory `Grill Stuff` cooked-dish cards now use the themed grilled-dish icon art as well, so dishes like `Crispy Perch Plate` show their fillet/plate grill miniature instead of the old generic chef icon.
- 2026-05-23: Local Monad shop test mode is now actually enabled for the owned API process too. The Node API loads `.env` / `.env.local` before reading flags, and local `.env.local` enables `HOOKLOOT_MONAD_SHOP_TEST_MODE_ENABLED=1` plus deterministic test MON drops so test MON rod purchases can complete without a real wallet payment; disable with the same env flags set to `0`.
- 2026-05-24: Shop rod grouping is now cleaned up for test QA. All base MON rods and bonus MON rods live in `Shop -> Rods`, `Monad Shop` only keeps gold packs, Auto Fishing Net, and cube-roll purchases, and test mode bypasses bonus-rod base gating so every MON rod can be bought during QA. The fake MON tx hash generator now forces an unsigned purpose hash, fixing invalid test hashes like `nft-rod-4`.

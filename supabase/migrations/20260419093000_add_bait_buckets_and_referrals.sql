ALTER TABLE public.players
  ADD COLUMN IF NOT EXISTS daily_free_bait INTEGER NOT NULL DEFAULT 30,
  ADD COLUMN IF NOT EXISTS daily_free_bait_reset_at TIMESTAMPTZ NULL,
  ADD COLUMN IF NOT EXISTS wallet_bait_bonus_claimed BOOLEAN NOT NULL DEFAULT FALSE,
  ADD COLUMN IF NOT EXISTS referrer_wallet_address TEXT NULL,
  ADD COLUMN IF NOT EXISTS referral_reward_granted BOOLEAN NOT NULL DEFAULT FALSE,
  ADD COLUMN IF NOT EXISTS rewarded_referral_count INTEGER NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS bonus_bait_granted_total INTEGER NOT NULL DEFAULT 0;

UPDATE public.players
SET daily_free_bait = 30
WHERE daily_free_bait IS NULL;
...
BEGIN
  ...
  IF _apply_wallet_bonus AND NOT player_row.wallet_bait_bonus_claimed THEN
    UPDATE public.players
